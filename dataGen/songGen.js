...
// const songId = Faker.random.number({min:9000000, max:10000000});

async function begin() {
  const writeStream = fs.createWriteStream('payload.csv');
  writeStream.write('id,title,artist,genre,album,albumart,songfile,createdat\n');
  for (let i = 1; i <= 1000; i++) {
    const ableToWrite = writeStream.write(`${Faker.random.number({min:9000000, max:10000000})},${generateTitle(hipster, 4)},${generateTitle(hipster, 3)},${generateTitle(hipster, 2)},${generateTitle(hipster, 2)},${Faker.image.imageUrl()},${Faker.image.imageUrl()},${Faker.random.number({min:9000000, max:10000000})}\n`);
    if (!ableToWrite) {
